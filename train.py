...
def semantic_segmentation_train(net_name, versions, use_half_training, model, device_id, old_lr, resume, total_epoch,
    ...
    try:
        ...
        for epoch in range(0 + resume, total_epoch + 1):
            ...
            log_handler.info('Train_Epoch = [%3d/%3d] | Iterations = %3d | LearningRate = %.8f | %s' % (epoch, total_epoch,
                            Iterations, optimizer.state_dict()['param_groups'][0]['lr'], str(datetime.datetime.now())))
            model.train()
            ...
            log_handler.info('Val_Epoch = [%3d/%3d] | Iterations = %3d | LearningRate = %.8f | %s' % (epoch, total_epoch, Iterations, optimizer.state_dict()['param_groups'][0]['lr'], str(datetime.datetime.now())))
            model.eval()
            torch.no_grad()

...
class Context_Guided_Network(nn.Module):
    ...
    def forward(self, input):
        ...
        concat1 = torch.cat([output0, inp1], 1)
        ...
        concat2 = torch.cat([output1, output1_0, inp2], 1)  
...
if __name__ == '__main__':
    # a_model = ChannelAttention(in_planes=35)
    # a_model = SpatialAttention()
    # a_model = CBAM(in_planes=35, planes=35)
    a_model = Context_Guided_Network(classes=2, M=3, N=21)
    print(a_model)   # 输出各个层以及卷积环节的参数
    in_x = torch.Tensor(1, 3, 512, 512)
    # in_x = torch.Tensor(1, 35, 256, 256)
    out111 = a_model(in_x)
    print(out111.size())
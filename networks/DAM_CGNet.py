...
class DAM_CGNet(nn.Module):
    def __init__(self, classes, M=3, dropout_flag=False):
        """
        args:
    ...
    def forward(self, input):
        ...
        # upsample segmenation map ---> the input size
...
class _DenseLayer(nn.Module):
    ...
    def bn_function(self, inputs):
        ...
        # inputs are the inputs of all previous layers.
...
if __name__ == '__main__':
    a_model = DAM_CGNet(classes=2, M=3)
    in_x = torch.Tensor(1, 1, 512, 512)
    out111 = a_model(in_x)
    print(out111)
    print(out111.size())
